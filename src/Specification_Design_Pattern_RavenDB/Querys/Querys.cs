...
namespace Specification_Design_Pattern_RavenDB.Querys
{
    ...
    {
        public static IRavenQueryable<T> Filtrar(IDocumentSession session, params ISpecification<T>[] especificacao)
        {
        ...
        public static IRavenQueryable<T> Filtrar(IRavenQueryable<T> query, params ISpecification<T>[] especificacao)
        {
            for (int i = 0; i < especificacao.Length; i++)
            {
                ISpecification<T>? item = especificacao[i];
